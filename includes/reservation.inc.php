...
require 'dbh.inc.php';
    ...
    $sql = "SELECT SUM(num_tables) FROM reservation WHERE reserv_date='$date' AND time_zone='$time'";
    $result = $conn->query($sql);
    if ($result->num_rows > 0) {
    while($row = $result->fetch_assoc()) {
    ...
    if($current_tables + $tables > $a_tables){
        header("Location: ../reservation.php?error3=full");
         ...
         $sql = "INSERT INTO reservation(f_name, l_name, num_guests, num_tables, reserv_date, time_zone, telephone, comment, user_fk) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)";
